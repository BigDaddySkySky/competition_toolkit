...
Write-Host "Creating a backup admin account (name: slatt)..." -ForegroundColor Cyan
...
New-LocalUser -Name slatt -Password $Password
net localgroup Administrators slatt /add
if ($DC) {
    Add-ADGroupMember -Identity "Domain Admins" -Members "slatt"
}
